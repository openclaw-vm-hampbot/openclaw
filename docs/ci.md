...
- The build-artifact job also persists content-fingerprinted `build-all` step outputs. Packaged plugin SDK declarations belong to the public/private `tsdown` groups. Native declarations used for local package-boundary checks have separate content records and never write package bridges or overwrite packaged declarations. See [test build ownership](/reference/test#shared-test-state-and-process-helpers). The built Doctor plugin-index proof reuses that exact `dist/` output instead of invoking the E2E harness's fallback TypeScript build a second time.
